...
*   toml
    ...
    {  "type": "battery",  "style": "powerline",  "powerline_symbol": "",  "foreground": "#193549",  "background": "#ffeb3b",  "background_templates": [    "{{if eq \"Charging\" .State.String}}#40c4ff{{end}}",    "{{if eq \"Discharging\" .State.String}}#ff5722{{end}}",    "{{if eq \"Full\" .State.String}}#4caf50{{end}}"  ],  "template": " {{ if not .Error }}{{ .Icon }}{{ .Percentage }}{{ end }} ",  "properties": {    "discharging_icon": " ",    "charging_icon": " ",    "charged_icon": " "  }}

    type: batterystyle: powerlinepowerline_symbol: foreground: "#193549"background: "#ffeb3b"background_templates:  - '{{if eq "Charging" .State.String}}#40c4ff{{end}}'  - '{{if eq "Discharging" .State.String}}#ff5722{{end}}'  - '{{if eq "Full" .State.String}}#4caf50{{end}}'template: " {{ if not .Error }}{{ .Icon }}{{ .Percentage }}{{ end }} "properties:  discharging_icon: " "  charging_icon: " "  charged_icon: " "

    type = "battery"style = "powerline"powerline_symbol = ""foreground = "#193549"background = "#ffeb3b"background_templates = [  "{{if eq \"Charging\" .State.String}}#40c4ff{{end}}",  "{{if eq \"Discharging\" .State.String}}#ff5722{{end}}",  "{{if eq \"Full\" .State.String}}#4caf50{{end}}"]template = " {{ if not .Error }}{{ .Icon }}{{ .Percentage }}{{ end }} "[blocks.segments.properties]discharging_icon = " "charging_icon = " "charged_icon = " "
