...
Display subversion information when in a subversion repository. Also works for subfolders. For maximum compatibility, make sure your `svn` executable is up-to-date (when branch or status information is incorrect for example).
...
*   toml
    ...
    {  "type": "svn",  "style": "powerline",  "powerline_symbol": "",  "foreground": "#193549",  "background": "#ffeb3b",  "properties": {    "fetch_status": true  }}

    type: svnstyle: powerlinepowerline_symbol: foreground: "#193549"background: "#ffeb3b"properties:  fetch_status: true

    type = "svn"style = "powerline"powerline_symbol = ""foreground = "#193549"background = "#ffeb3b"[blocks.segments.properties]fetch_status = true
...
As doing multiple [subversion](https://subversion.apache.org/)
 calls can slow down the prompt experience, we do not fetch information by default. You can set the following properties to `true` to enable fetching additional information (and populate the template).
