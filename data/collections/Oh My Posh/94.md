...
*   toml
    ...
    {  "background": "p:sky",  "foreground": "p:white",  "powerline_symbol": "",  "properties": {    "api_key": "<YOUR_API_KEY>",    "username": "<LASTFM_USERNAME>",    "http_timeout": 20000  },  "style": "powerline",  "template": " {{ .Icon }}{{ if ne .Status \"stopped\" }}{{ .Full }}{{ end }} ",  "type": "lastfm"}

    background: p:skyforeground: p:whitepowerline_symbol: properties:  api_key: <YOUR_API_KEY>  username: <LASTFM_USERNAME>  http_timeout: 20000style: powerlinetemplate: ' {{ .Icon }}{{ if ne .Status "stopped" }}{{ .Full }}{{ end }} 'type: lastfm

    background = "p:sky"foreground = "p:white"powerline_symbol = ""style = "powerline"template = ' {{ .Icon }}{{ if ne .Status "stopped" }}{{ .Full }}{{ end }} 'type = "lastfm"[blocks.segments.properties]api_key = "<YOUR_API_KEY>"username = "<LASTFM_USERNAME>"http_timeout = 20_000
